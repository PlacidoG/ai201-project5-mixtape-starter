...
### Issue #3 — "Same song shows up twice in search" (latent defect — hardened defensively)
- **How I reproduced it — and why it doesn't reproduce:** I tried to reproduce it directly: `search_songs("Crown Heights")` against a seeded 3-tag song returned the song **once**, not 3×, and the checked-in test `test_search_no_duplicates_multi_tag_song` (expects `1`) **passes**. So the reported user-visible symptom does not occur on this SQLAlchemy version. To confirm the mechanism was nonetheless real, I ran the raw column query `db.session.query(Song.id).outerjoin(song_tags, …)` which returned **3** rows — the fan-out exists at the SQL level but is collapsed before it reaches the API.
- **How I found the root cause:** Read `search_songs` in `services/search_service.py`. It builds `db.session.query(Song).outerjoin(song_tags, Song.id == song_tags.c.song_id).filter(title/artist ilike …)`. The join emits one row per matching `song_tags` row, so an N-tag song produces N rows. The reason it doesn't surface: SQLAlchemy's legacy `Query` API (`db.session.query(Entity)`) automatically de-duplicates whole-entity results by primary-key identity, masking the fan-out. The certainty came from the raw-column query showing 3 rows while the entity query showed 1 — pinpointing the join as the latent cause.
- **The root cause:** An unnecessary `outerjoin(song_tags)` in the search query. It contributes nothing (the filter is only on `Song.title`/`Song.artist`, and tags are loaded separately via the `Song.tags` relationship inside `to_dict()`), but it fans the result set out to one row per tag. Today it's masked by legacy-Query entity uniquing; it would become a visible duplicate bug the moment the query is migrated to SQLAlchemy 2.0 `select()` style, where uniquing is not automatic unless `.unique()` is called.
- **My fix and side-effect check:** Removed the purposeless `.outerjoin(song_tags, …)` entirely (root-cause fix, rather than masking it with `.distinct()`), and pruned the now-unused `Tag`/`song_tags` import to `from models import Song` ([services/search_service.py:8](services/search_service.py#L8), [services/search_service.py:25-34](services/search_service.py#L25)). Side-effect check — the other function in the file, `get_song`, only uses `Song`, so the import change is safe; all 5 `tests/test_search.py` cases still pass; and I confirmed via harness that a 3-tag song still returns its full `tags` list (`['rap','hip-hop','boom bap']`), i.e. dropping the join did not drop tag data because `to_dict()` reads the relationship, not the join.

### Issue #4 — No notification when a friend rates your shared song
- **How I reproduced it:** No test file covers notifications, so I used the in-memory harness. A `sharer` shared a song; a different `rater` called `rate_song(rater_id, song_id, 5)`; then `get_notifications(sharer_id)` returned length `0` both before and after — confirming rating produces no notification, while the seed data and the playlist path (`add_to_playlist`) do create notifications.
- **How I found the root cause:** Traced `POST /songs/<id>/rate` → `routes/songs.py` → `notification_service.rate_song`. Read the whole function: it validates the song and rater, upserts the `Rating`, commits, and returns — with no `create_notification` call anywhere. I then compared it to its sibling in the same file, `add_to_playlist` ([notification_service.py:64-70](services/notification_service.py#L64)), which *does* call `create_notification(user_id=song.shared_by, …)`. That side-by-side made it certain: the two "someone interacted with your song" paths were inconsistent, and the rating path simply omitted the notification.
- **The root cause:** `rate_song` never created a notification for the song's owner. It was a missing step, not a wrong one — the function persisted the rating correctly but had no equivalent of `add_to_playlist`'s notify-the-sharer logic.
- **My fix and side-effect check:** Added a `create_notification(...)` call after the commit, gated to mirror `add_to_playlist`'s semantics: only on a **new** rating (`existing is None`, so score edits don't re-notify) and only when `song.shared_by != user_id` (no self-notification). Reused the already-fetched `song` and `rater` and the same-module `create_notification`, with `notification_type="song_rated"` ([services/notification_service.py:110-118](services/notification_service.py#L110)). Side-effect check via harness: a new rating by a non-sharer yields exactly `1` `song_rated` notification; a subsequent re-rating leaves it at `1` (no spam); a self-rating leaves it at `1` (no self-notify). The `Rating` upsert and unique-constraint behavior are unchanged, and the full suite showed no regressions.

### Issue #5 — The last song in a playlist never shows up
- **How I reproduced it:** Ran `python -m pytest tests/test_playlists.py` before touching code. `test_playlist_returns_all_songs` seeds a 5-song playlist and asserts `len == 5`; it failed returning `4`. `test_playlist_returns_songs_in_order` also failed, missing `"Track 5"`. Real-app equivalent: `GET /playlists/<id>/songs` on any non-empty playlist returns one fewer song than were added.
