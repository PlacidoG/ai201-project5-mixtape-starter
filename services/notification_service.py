...
def rate_song(user_id: str, song_id: str, score: int) -> Rating:
    ...
    # Notify the person who originally shared the song, but only on a new
    # rating (not score edits) and not when they rated their own song —
    # mirroring add_to_playlist's notification behavior.
    if existing is None and song.shared_by != user_id:
        create_notification(
            user_id=song.shared_by,
